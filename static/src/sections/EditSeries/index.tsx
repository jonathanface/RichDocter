...
export const EditSeries = () => {
    ...
    if (storiesList) {
      setAvailableStories(
        storiesList.filter(
          (story) =>
            !seriesBuild.stories ||
            !seriesBuild.stories.some((s) => s.story_id === story.story_id),
        ),
      );
    }
    ...
    const fetchSeries = async () => {
      try {
        ...
        const editingSeriesBuild: EditSeriesForm = {
          ...
          image_url: data.image_url?.length
            ? data.image_url
  ...
  const handleSubmit = async () => {
    ...
    if (
      seriesBuild.series_description &&
      !seriesBuild.series_description?.trim().length
    ) {
      ...
      if (storiesList) {
        json.stories.forEach((story) => {
          ...
          if (standaloneVersionIDX !== -1) {
            setStoriesList(
              storiesList.filter((_, idx) => idx !== standaloneVersionIDX),
            );
          }
        });
      }
  ...
  const removeStory = async (
    ...
    if (conf) {
      ...
      try {
        ...
        const removedStory = seriesBuild.stories?.find(
