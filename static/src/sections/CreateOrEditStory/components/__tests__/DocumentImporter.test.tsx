...
describe('DocumentImporter', () => {
  it('renders upload prompt when no file selected', () => {
    render(
      <DocumentImporter importFile={null} onFileSelected={vi.fn()} />
    );
  ...
  it('shows selected filename when file is provided', () => {
    ...
    render(
      <DocumentImporter importFile={file} onFileSelected={vi.fn()} />
    );
  ...
  it('calls onFileSelected(null) when remove is clicked', () => {
    ...
    const file = new File(['content'], 'test.txt', { type: 'text/plain' });
    render(
      <DocumentImporter importFile={file} onFileSelected={onFileSelected} />
    );
  ...
  it('renders in disabled state', () => {
    const { container } = render(
      <DocumentImporter importFile={null} onFileSelected={vi.fn()} disabled />
    );
