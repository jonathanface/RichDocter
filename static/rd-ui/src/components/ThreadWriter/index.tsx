...
export interface ThreadWriterProps {
  story: Story
}
...
export const ThreadWriter = (props: ThreadWriterProps) => {
  ...
  const { deselectStory, setCurrentStory } = useCurrentStoryContext();
  ...
  const getAllAssociations = useCallback(async () => {
    ...
    return fetch("/api/stories/" + props.story.story_id + "/associations")
      .then((response) => {
        if (response.ok) {
  ...
  }, [createDecorators, editorState, props.story.story_id]);
  ...
  }, [props.story.story_id, selectedChapter.id, setAlertState]);
  ...
  useEffect(() => {
    ...
    if (!associationsLoaded && blocksLoaded.current) {
      getAllAssociations();
    }
  ...
  }, [associationsLoaded, blocksLoaded.current, setIsLoaderVisible]);

  useEffect(() => {
    getChapterDetails();
    blocksLoaded.current = false;
  }, [selectedChapter.id, props.story.story_id, getChapterDetails]);
  ...
  const syncBlockOrderMap = (blockList: BlockMap) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const response = await fetch(
          "/api/stories/" + props.story.story_id + "/orderMap",
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const updateAssociationsOnServer = (
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const response = await fetch(
          "/api/stories/" + props.story.story_id + "/associations",
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const onStoryTitleEdit = async (event: React.SyntheticEvent) => {
    ...
    if (target.value !== props.story.title && target.value.trim() !== "") {
      const updatedStory: Story = { ...props.story };
  ...
  const onChapterTitleEdit = async (event: React.SyntheticEvent) => {
    ...
    if (target.value !== selectedChapter.title) {
      if (
        ...
        props.story.chapters.forEach((chapter, idx) => {
  ...
  return (
    <div>
      <AssociationUI
        ...
        storyID={props.story.story_id as string}
      ...
      <div className={docStyles.titleInfo}>
        <h2>
          <EditableText
            textValue={props.story.title as string}
