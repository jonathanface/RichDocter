...
export const AssociationDecoratorPlugin = ({
    ...
    (
      ...
      nodesToCheck.forEach((node) => {
        ...
        if (!currentAssoc) {
          ...
          if (
            currentAssoc.short_description !== node.getShortDescription() ||
            currentAssoc.portrait !== node.getPortrait()
          ) {
            obsoleteNodes.push(node);
          }
          ...
          const enteredText = currentAssoc.case_sensitive
            ? nodeText
  ...
  const processObsoleteAssociations = (node: AssociationInlineNode): void => {
    ...
    const format: number = (node as any).getFormat
      ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (node as any).getFormat()
      : 0;
    ...
    (
      ...
      textNodes.forEach((textNode) => {
        ...
        if (
          ...
          selection.anchor.getNode() === textNode
        ) {
