...
const Sidebar = (props) => {
      ...
      Array.from(stories.entries()).map(([key, story]) => {
        if (seriesTitle && key === seriesTitle) {
          story.forEach((entry) => {
            if (entry.key === bookTitle) {
              ...
              if (chapterIndex >= 0) {
                ...
                entry.chapters.push({
                  chapter_title: newOrDeletedChapter,
                  chapter_num: parseInt(entry.chapters.length + 1),
                });
              }
            }
          });
          ...
          if (chapterIndex >= 0) {
            ...
            story["chapters"].push({
              chapter_title: newOrDeletedChapter,
              chapter_num: parseInt(story.chapters.length + 1),
            });
          }
        }
        return [key, story];
        ...
        .then((response) => {
          if (response.ok) {
            ...
            history.pushState(
              { bookTitle },
              "created chapter",
              "/story/" + encodeURIComponent(bookTitle) + "?chapter=" + chapterNum + "&title=" + title
            );
            return;
          }
  ...
  const deleteChapter = (event, story, seriesTitle, chapterTitle, chapterNum) => {
    event.preventDefault();
    event.stopPropagation();
    const chaptersList = seriesTitle
      ? stories.get(seriesTitle)[stories.get(seriesTitle).findIndex((e) => e.key === story)].chapters
      : stories.get(story).chapters;
    ...
    <nav className="menu-container">
      <span className="checkbox-container">
        <input
          ...
          type="checkbox"
          onChange={() => {
            dispatch(flipChapterMenuOpen());
          }}
          checked={isOpen}
        />
        <span className="menu-content">
          <TreeView
            aria-label="documents navigator"
            onNodeSelect={(event, nodeId) => {
              updateMenuExpandedNodes(nodeId);
            }}
            defaultCollapseIcon={<ExpandMoreIcon />}
            defaultExpandIcon={<ChevronRightIcon />}
            expanded={expanded}
            defaultExpanded={["story_label"]}>
            {!isLoggedIn ? (
